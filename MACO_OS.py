    ...
    def search(self, discretization):
        if discretization[0] in self.ait.shooting:
            if discretization[1] in self.ait.shooting[discretization[0]]:
                self.pref = self.ait.shooting[discretization[0]][discretization[1]]["preference"]
                self.fail = self.ait.shooting[discretization[0]][discretization[1]]["fail"]
                action = 0
        elif discretization[2] in self.ait.passing:
            if discretization[3] in self.ait.passing[discretization[2]]:
                self.pref = self.ait.passing[discretization[2]][discretization[3]]["preference"]
                self.fail = self.ait.passing[discretization[2]][discretization[3]]["fail"]
                action = 1
        elif discretization[4] in self.ait.dribbling:
            if discretization[5] in self.ait.dribbling[discretization[4]]:
                self.pref = self.ait.dribbling[discretization[4]][discretization[5]]["preference"]
                self.fail = self.ait.dribbling[discretization[4]][discretization[5]]["fail"]
                action = 2
        else:
